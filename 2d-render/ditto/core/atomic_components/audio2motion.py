...
class Audio2Motion:  # kp - keypoint
    def __init__(
        self,
        lmdm_cfg,
    ):
        self.lmdm = LMDM(**lmdm_cfg)  # ? Модель Latent Molecular Diffusion Model
    ...
    def setup(
        ...
        self.overlap_v2 = overlap_v2  # 70 поум
        self.seq_frames = self.lmdm.seq_frames
        self.valid_clip_len = self.seq_frames - self.overlap_v2  # 80 - 70 = 10
        logger.info(f"-------- SETUP {self.overlap_v2} {self.seq_frames} {self.valid_clip_len} --------")

        # for fuse
        self.online_mode = online_mode  # ? Если online, то длина слияния по идее меньше
        if self.online_mode:
            self.fuse_length = min(self.overlap_v2, self.valid_clip_len)  # ? 10 поум
        else:
            self.fuse_length = self.overlap_v2
        logger.info(f"-------- SETUP {self.online_mode} {self.fuse_length} --------")
    ...
    def _fuse(self, res_kp_seq, pred_kp_seq):  # по сути слияние двух кусков - предыдущего (res: result) и текущего (pred: prediction)
        ## ========================
        ## offline fuse mode
        ## last clip:  -------
        ## fuse part:    *****
    ...
    def _update_kp_cond(self, res_kp_seq, idx):
        if self.fix_kp_cond == 0:  # 不重置 # без сброса
            self.kp_cond = res_kp_seq[:, idx-1]
        elif self.fix_kp_cond > 0:
            if self.clip_idx % self.fix_kp_cond == 0:  # 重置 # перезагрузить
                self.kp_cond = self.s_kp_cond.copy()  # 重置所有 # сбросить все
