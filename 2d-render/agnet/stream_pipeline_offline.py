...
from agnet.core.atomic_components.writer import VideoWriterByImageIO
...
class StreamSDK:
    ...
    def stream_frames(self):
        while not self.stop_event.is_set():
            try:
                ...
                logger.info("STREAMER GET")
                if item is not None:
                    yield item
            except queue.Empty:
                logger.info("STREAMER QUEUE EMPTY")
                continue
            except Exception as e:
                logger.info("STREAMER STOP EVENT")
    ...
    def _audio2motion_offline(self):
        ...
        while not self.stop_event.is_set():
            ...
            if item is None:
                logger.info("ITEM IS NONE MEAN LAST, BREAK")
                break
            ...
            logger.info(f"NUM FRAME IS {num_frames}")
            idx = 0
            res_kp_seq = None
            # pbar = tqdm(desc="dit")
            ...
            logger.info(f"START AUDIO2MOTION SMO")
            res_kp_seq = self.audio2motion._smo(res_kp_seq, 0, res_kp_seq.shape[1])
            logger.info(f"END AUDIO2MOTION SMo")

            x_d_info_list = self.audio2motion.cvt_fmt(res_kp_seq)

            gen_frame_idx = 0
            logger.info(f"NUMBER OF FRAMES IS {len(x_d_info_list)}")
    ...
    def _audio2motion_worker(self):
        ...
        while not self.stop_event.is_set():
            ...
            if not is_end and item_buffer.shape[0] < valid_clip_len:
                ...
                item_buffer = np.zeros((0, aud_feat_dim), dtype=np.float32)
                logger.info(f"GOT LENGTH {valid_clip_len}, START RENDER")
            ...
            while True:
                ...
                if res_kp_seq_valid_start is None:
                    ...
                    logger.info(f"NUMBER OF FRAMES IS {x_d_info_list}")
